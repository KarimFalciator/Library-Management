...
#take off none as t_id once testing after login
    ...
    def create_returned_tab(self):
        ...
        columns = ('ref', 's_id', 'r_id', 'b_date', 'd_date', 'r_date')
        self.current_tree = ttk.Treeview(return_tab, columns=columns, show='headings')
        ...
        # Add a vertical scrollbar
        scrollbar = ttk.Scrollbar(return_tab, orient='vertical', command=self.current_tree.yview)
        scrollbar.grid(row=0, column=4, sticky='ns')
        self.current_tree.configure(yscrollcommand=scrollbar.set)

        # Add Borrowed ResourceForm
        self.add_borrowed_CTklable = ctk.CTkLabel(return_tab, text="Add a new borrowed:")
        self.add_borrowed_CTklable.grid(row=1, column=0, padx=5, pady=0, sticky='n')

        self.h_s_id_CTklabel = ctk.CTkLabel(return_tab, text="Student ID:")
        self.h_s_id_CTklabel.grid(row=2, column=0, padx=5, pady=5)
        self.h_s_id_entry = ctk.CTkEntry(return_tab)
        self.h_s_id_entry.grid(row=3, column=0, padx=5, pady=5)

        self.h_r_id_CTklabel = ctk.CTkLabel(return_tab, text="Resource ID:")
        self.h_r_id_CTklabel.grid(row=2, column=1, padx=5, pady=5)
        self.h_r_id_entry = ctk.CTkEntry(return_tab)
        self.h_r_id_entry.grid(row=3, column=1, padx=5, pady=5)

        self.h_days_CTklabel = ctk.CTkLabel(return_tab, text="Days borrowed:")
        self.h_days_CTklabel.grid(row=2, column=2, padx=5, pady=5)
        self.h_days_entry = ctk.CTkEntry(return_tab)
        self.h_days_entry.grid(row=3, column=2, padx=5, pady=5)

        self.add_borrowed = ctk.CTkButton(return_tab, text="Add Borrowed resource", command=self.add_borrowed_resource)
        self.add_borrowed.grid(row=4, column=1, columnspan=1, pady=10)
